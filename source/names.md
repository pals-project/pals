...
The full form of a name match string is
```{code} text
...
- A set of lattice element names using using ranges, unions, and intersections
  as explained in [](#s:element.sets).
- Controller name
- A value associated with a [MetaP](#s:meta.params) string component.
...
Q1:Q2>length
```
The first line will match to all `Marker` elements that are in the range from `Q1` to `Q2`.
The second line matches to the lengths of all elements between `Q1` and `Q2`.
...
Character tokens inside of any `(...)`, `[...]` or `{...}` parenthesis group are ignored 
in terms isolating a base name or for matching to ranges, unions, and intersections.
For example:
```{code} text
Qa,Qb
Q\d{2,3}
```
In the first example, the comma appears outside of any parenthesis group and is considered
...
to be part of the PRE2 string. In this case the string will match to the letter `Q` followed
by either 2 or 3 digits. For example, `Qab3`.
...
Examples:
```{code} yaml
Marker::bpm.        # Marker elements named bpm.
Controller::cdt     # The controller element named cdt.
...
(since a dot matches to any single character). The second line will match to all `Controller`s
whose four-character name starts with `cd` and ends with `t`. The third example matches to all
lattice elements with Metap.alias equal to "black".
...
where
```{code} yaml
{element-name}                  # Element name match.
{parameter-group}               # Parameter group name (for grouped parameters).
{sub-group1}. ... .{sub-groupN} # Subgroups if they exist.
{parameter}                     # Parameter name.
```
The `{element-name}` component is any element name match as described above — including its
kind (`::`), instance (`#`), branch (`>>`), and lattice (`>>>`) qualifiers. The parameter path
